...
`define DBG_TRX_AXI
`endif
module trx_axi // only for 32-bit address, 32-bit data, 4-bit id
(
                    input  wire                     ARESETn
     ,              input  wire                     ACLK
     ...
     `else
     , `DBG_TRX_AXI output wire [ 3:0]              AWLEN
     , `DBG_TRX_AXI output wire [ 1:0]              AWLOCK
     `endif
     ...
     `else
     , `DBG_TRX_AXI output wire [ 3:0]              ARLEN
     , `DBG_TRX_AXI output wire [ 1:0]              ARLOCK
     `endif
